...
export class RubyProcess extends EventEmitter {
	...
	public constructor(mode: Mode, args: any) {
			...
			if (this.domErrors.length) {
				...
				if (
					!(
						...
						includes(
							this.domErrors[0].error,
							"elements closed character '/' and '>' must be connected",
							0
						)
					)
				)
					this.emit(
						'debuggerOutput',
						'Debugger failed to parse: ' +
							this.domErrors[0].error +
							'\nFor: ' +
							this.buffer.slice(0, 20)
					);
				if (
					this.buffer.indexOf('<eval ') >= 0 &&
					(includes(this.domErrors[0].error, 'attribute space is required', 0) ||
						includes(
							this.domErrors[0].error,
							"elements closed character '/' and '>' must be connected",
					...
					for (let nodeIdx = 0; nodeIdx < node.childNodes.length; nodeIdx++) {
						...
						if (childNode.attributes && childNode.attributes.length) {
							for (let attrIdx = 0; attrIdx < childNode.attributes.length; attrIdx++) {
								attributes[childNode.attributes[attrIdx].name] =
									childNode.attributes[attrIdx].value;
							}
						}
		...
		if (mode == Mode.launch) {
			...
			this.debugprocess = childProcess.spawn(
				runtimeExecutable,
				[...runtimeArgs, '--', args.program, ...(args.args || [])],
				{ cwd: processCwd, env: processEnv }
			);
